...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (!username || !password) return res.status(400).json({ error: "Manque username ou password" });
    if (data.users[username]) return res.status(400).json({ error: "Utilisateur existe déjà" });
